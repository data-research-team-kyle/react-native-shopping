...
const TransactionHistoryItem: React.FC<TransactionHistoryItemProps> = ({ image, name, date, type, amount, onPress }) => {
  const { dark } = useTheme();
    ...
    <TouchableOpacity
      ...
      <View style={styles.viewLeftContainer}>
        ...
        <View>
          ...
          <Text style={[styles.date, { color: dark ? COLORS.grayscale400 : COLORS.grayscale700 }]}>{date}</Text>
        </View>
      </View>
      <View style={styles.viewRightContainer}>
        <Text style={[styles.amount, { color: dark ? COLORS.white : COLORS.greyscale900 }]}>{amount}</Text>
        <View style={styles.typeContainer}>
          <Text style={[styles.type, { color: dark ? COLORS.grayscale400 : COLORS.grayscale700 }]}>{type}</Text>
          <Image
            source={ type === "Purchase Expense" ? icons.arrowUpSquare : icons.arrowDownSquare }
            resizeMode="contain"
            style={[styles.typeIcon, { tintColor: type === "Purchase Expense" ? COLORS.red : COLORS.blue }]}
          />
        </View>
      </View>
    </TouchableOpacity>
