...
const CartCard: React.FC<CartCardProps> = ({ name, image, price, rating, numReviews, onPress, size, color }) => {
  const { dark } = useTheme();
    ...
    <TouchableOpacity
      ...
      <View style={styles.columnContainer}>
        ...
        <View style={styles.viewContainer}>
          ...
          <Text style={[styles.location, { color: dark ? COLORS.greyscale300 : COLORS.grayscale700 }]}>{" "}{rating}  ({numReviews})</Text>
          {size && (
            <Text style={[styles.location, { color: dark ? COLORS.greyscale300 : COLORS.grayscale700 }]}>   | {" "} Size= {size}  </Text>
          )}
        </View>
        <View style={styles.bottomViewContainer}>
          ...
          <View style={[styles.qtyContainer, { backgroundColor: dark ? COLORS.dark3 : COLORS.silver}]}>
            <TouchableOpacity onPress={decrease}>
              <Text style={[styles.qtyText, { color: dark ? COLORS.white : COLORS.primary}]}>-</Text>
            </TouchableOpacity>
            <Text style={[styles.qtyNum, { color: dark ? COLORS.white : COLORS.primary }]}>{quantity}</Text>
