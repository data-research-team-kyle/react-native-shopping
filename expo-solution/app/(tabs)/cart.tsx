...
const Cart: React.FC = () => {
  ...
  const handleRemoveBookmark = () => {  
    if (selectedBookmarkItem) {
      const updatedCartProduct = myCartProducts.filter((product: any) => product.id !== selectedBookmarkItem.id);
  ...
  const renderHeader = () => (
    <TouchableOpacity style={styles.headerContainer}>
      <View style={styles.headerLeft}>
        ...
        />
        <Text style={[styles.headerTitle, { color: dark ? COLORS.white : COLORS.greyscale900 } ]}>My Cart</Text>
      </View>
      <TouchableOpacity>
        <Image
  ...
  */
  const renderContent = () => {
    const filteredProducts = myCartProducts.filter((product: any) => selectedCategories.includes("0") || selectedCategories.includes(product.categoryId));
    ...
    <SafeAreaView style={[styles.area, { backgroundColor: colors.background }]}>
      ...
      <RBSheet
        ...
        customStyles={{
          ...
          <Text style={[styles.bottomSubtitle, { color: dark ? COLORS.white : COLORS.black }]}>Remove from Cart?</Text>
